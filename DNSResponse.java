...
public class DNSResponse {
	...
	private int queryID; /*
						 * this is for the response it must match the one in the
						 * request
						 */
	private byte header[];
	...
	private boolean hasIPaddr = false;

	/**
	 * Prints out the DNS response information
	 */
	void dumpResponse() {
	...
	/**
	 * If the answer section is empty but the address of the next name server
	 * (from the additional section) is known, then return this address, which
	 * can be used to re-query the domain name.
	 * 
	 * @return IP address of name server to send the query to next, otherwise
	 *         null.
	 */
	...
	public DNSResponse(byte[] data, int len, DNSQuery q) {
		...
		// Set finishedSearchForIPaddr to true if we are done
		setHasIPaddr();

		// Set domainNameInfo which can be passed to DNSlookup class
		if (hasIPaddr) {
			InetAddress ipAddress = getIPaddr();
			int ttl = answerList[0].ttl;
			domainNameInfo = new DomainNameInfo(aaFQDN, ipAddress, ttl);
		}
	}

	/**
	 * If we received an IP address in answer section of response, then set
	 * obtainedIPaddr to true. Otherwise false.
	 */
	private void setHasIPaddr() {
		hasIPaddr = ((answerCount > 0) && (answerList[0].type == 1));
	}
